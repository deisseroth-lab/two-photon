...
def main():
    ...
    try:
        ...
        logger.info(
            'Found stim channel "%s", enabled=%s',
            stim_channel_name,
            stim_channel_enabled,
        )
    ...
    if args.preprocess or args.run_suite2p:
        ...
        if args.preprocess:
            preprocess(
                ...
                dirname_output,
                fname_csv,
                fname_uncorrected_hdf5,
                fname_hdf5,
                mdata,
                args.artefact_buffer,
                args.artefact_shift,
                args.channel,
                stim_channel_name,
                args.settle_time,
...
def preprocess(
    ...
    if stim_channel_name:
        ...
        df_artefacts = artefacts.get_bounds(
            df_voltage,
            frame_start,
            size,
            stim_channel_name,
    ...
    if system == "Windows":
        ...
        if os.path.isdir(local_location):
            ...
            cmd = [
                ...
                "-avh",
                str(local_location),
                str(backup_location / local_location.name),
            ]
...
def run_suite2p(hdf5_list, dirname_output, mdata):
    ...
    fs_param = 1.0 / (mdata["period"] * z_planes)
    ...
    logger.info(
        "Running suite2p on files:\n%s\n%s",
        "\n".join(str(f) for f in hdf5_list),
        params,
    )
...
def parse_args():
    ...
    group.add_argument(
        ...
        type=str,
        help=(
            "Name of a recording, given as a slash separated id of SESSION/RECORDING/OPTIONAL_PREFIX "
    ...
    group.add_argument(
        ...
        default=[],
        help=(
            "Name of one or more already preprocessed recordings to merge during suite2p.  "
    ...
    group.add_argument(
        ...
        type=float,
        default=20,
        help="Time to exclude following calculated artefact",
    )
    group.add_argument(
        "--artefact_shift",
        type=float,
        default=1,
        help="Time to shift artefact position from nominal.",
    )

    group.add_argument(
        "--backup_data",
        action="store_true",
        help="Backup all input data (post-ripping)",
    )
    group.add_argument(
        "--backup_hdf5",
        action="store_true",
